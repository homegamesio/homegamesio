...
const makePost = (endpoint, payload, notJson)  => new Promise((resolve, reject) => {
    ...
    xhr.setRequestHeader("User-Agent", "PostmanRuntime/7.26.8");
...
const modals = {
    'support': {
        render: () => {
            ...
            const emailText = simpleDiv('If you want a response, leave your email below');
            
            const emailForm = document.createElement('input');
            emailForm.type = 'text';
            emailForm.setAttribute('placeHolder', 'Email (optional)');
